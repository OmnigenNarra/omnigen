	...
	_mousePressTitleWidget(NULL)
{
	...
	_topLayout->setContentsMargins(2, 2, 2, 2);
    _topLayout->setSpacing(0);

	auto* w = new QWidget;
	w->setObjectName("sectionWidget");
    //w->setStyleSheet(getDockHeaderStyle());
	w->setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Fixed);

	w->setLayout(_topLayout);
	l->addWidget(w);

    _minimizeButton = new QPushButton();
    _minimizeButton->setObjectName("sectionButton");
	_minimizeButton->setCheckable(true);
	_minimizeButton->setChecked(false);
    _minimizeButton->setFlat(true);
    _minimizeButton->setIcon(QIcon("Resources/Icons/icon_branch_open.png"));
    _minimizeButton->setToolTip(tr("Minimize"));
    _topLayout->addWidget(_minimizeButton, 0);
	...
	_tabsMenuButton->setObjectName("sectionButton");
	...
	//_tabsMenuButton->setStyleSheet("background-color: transparent;");
	...
	_closeButton->setObjectName("sectionButton");
	_closeButton->setFlat(true);
	_closeButton->setIcon(QIcon("Resources/Icons/icon_close.png"));
    ...
    auto* collapsableWidget = new QWidget();
	collapsableWidget->setLayout(_contentsLayout);

	l->addWidget(collapsableWidget, 1);

    connect(_minimizeButton, &QPushButton::clicked, this, [this, collapsableWidget, w]()
        {
            if (_minimizeButton->isChecked())
            {
                collapsableWidget->hide();
                _minimizeButton->setIcon(QIcon("Resources/Icons/icon_branch_closed.png"));
                auto* sp = findParentSplitter(this);
                if (sp->orientation() == Qt::Vertical)
                {
                    QList<int> newList = sp->sizes();
                    int idx = sp->indexOf(this);
                    _maximizedSize = newList.at(idx);
                    if (idx != newList.size() - 1)
                    {
                        newList.replace(idx, w->height());
                        newList.replace(newList.size() - 1, newList.at(newList.size() - 1) + (_maximizedSize - w->height()));
                        sp->setSizes(newList);
                    }
                }
            }
            else
            {
                ...
                if (sp->orientation() == Qt::Vertical)
                {
                    ...
                    if (idx != newList.size() - 1)
                    {
                        newList.replace(idx, _maximizedSize);
                        newList.replace(newList.size() - 1, newList.at(newList.size() - 1) - (_maximizedSize - w->height()));
                        sp->setSizes(newList);
                    }
                }
...
{
	_closeButton->setStyleSheet(data.titleWidget->styleSheet());
...
void SectionWidget::toggleCloseButtonVisibility(bool visible)
{
