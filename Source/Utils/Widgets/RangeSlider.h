...
class RangeSlider : public QSlider {
    Q_OBJECT
public:
    RangeSlider(QWidget* parent = nullptr, Qt::Orientation ot = Qt::Horizontal)
        : QSlider(ot, parent)
    {
...
protected:
    ...
    void paintEvent(QPaintEvent* ev) override {
        ...
        int rightLast = 0;
        QRect rr;
        painter.drawText(rect(), Qt::TextDontPrint, labels.empty() ? QString::number(9999) : labels.at(longestLabelKey), &rr);

        auto ti = tickInterval();
        auto min = this->minimum();
        auto max = this->maximum();
        for (int v = this->minimum(); v <= this->maximum(); v += ti) {
            ...
            //int len = style->pixelMetric(QStyle::PM_SliderLength, &slider, this);
            int len = 0;
            int left = QStyle::sliderPositionFromValue(minimum(), maximum(), v, available) + len;
            QString vs = labels.empty() ? QString("%L1").arg(v) : labels.at(v);
            ...
            if (orientation() == Qt::Vertical)
            {
                ...
                int vv = this->minimum() - v + this->maximum();
                vs = labels.empty() ? QString("%L1").arg(vv) : labels.at(vv);
            }
            QRect r;
            //if (left < available && rightLast <= left - len)
            {
                rr.setX(pos.x());
                rr.setY(pos.y());
                painter.drawText(rr, Qt::TextDontPrint, vs, &r);
                painter.drawText(pos, vs);
            }
    ...
    int longestLabelKey = 0;

    template<typename T>
    friend class RangeField;
};