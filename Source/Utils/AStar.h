...
public: // methods
	...
	unsigned int SearchStep()
	{
		...
		// search...
		//Balgy's Change: If the state has failed you need to call FreeAllNodes() yourself.
		if (m_OpenList.empty())
		{
			FreeUnusedNodes();
			m_State = SEARCH_STATE_FAILED;
...
public:
	...
	virtual float goalDistanceEstimate(const T& nodeGoal) const = 0;
	// Returns true if this node is the goal node
	virtual bool isGoal(const T& nodeGoal) const = 0;
	...
	virtual float getCost(const T& successor) const = 0;
	// Returns true if this node is the same as the rhs node
	virtual bool isSameState(const T& rhs) const = 0; 
};
