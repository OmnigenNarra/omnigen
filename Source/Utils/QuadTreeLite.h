...
namespace tml {
    ...
    public:
        ...
        std::vector<const node_type*> find_all_nearest(COOR_TYPE x, COOR_TYPE y, COOR_TYPE radius) const {
            std::vector<const node_type*> result;
            search(x, y, radius, result);
            return result;
        }

        std::multimap<float, const node_type*> map_all_nearest(COOR_TYPE x, COOR_TYPE y, COOR_TYPE radius) const {
            std::multimap<float, const node_type*> result;
            search(x, y, radius, result);
            return result;
        }
        ...
        template<typename T>
        bool search(COOR_TYPE x, COOR_TYPE y, COOR_TYPE radius, T& result) const {
            result.clear();
            search(root, x, y, radius, result);
            return !result.empty();
        }

        //search for nodes in a specified rectangle along segment
        template<typename T>
        bool search(const Segment2D& seg, COOR_TYPE radius, T& result) const {
            result.clear();
            search(root, seg, radius, result);
            return !result.empty();
        }
    ...
    private:
        ...
        bool intersect(const node_type* n, const Segment2D& seg, COOR_TYPE r) const {
            COOR_TYPE x0 = (n->x + n->x1) / 2;
            COOR_TYPE y0 = (n->y + n->y1) / 2;
            COOR_TYPE distToSeg = seg.dist({ x0, y0 });
            return distToSeg <= r + std::hypot(x0 - n->x, y0 - n->y);
        }

        template<typename T>
        void search(const node_type* n, COOR_TYPE x, COOR_TYPE y, COOR_TYPE radius, T& result) const {
            if (n != NULL) {
                if (n->leaf) {
                    if (float d = std::hypot(n->x - x, n->y - y); d <= radius) {
                        if constexpr (std::is_same_v<T, std::vector<const node_type*>>) {
                            result.push_back(n);
                        }
                        else if constexpr (std::is_same_v<T, std::multimap<float, const node_type*>>) {
                            result.insert({d, n});
                        }
                        else {
                            static_assert(std::is_same_v<T, std::false_type>, "QuadTreeLite: Unhandled search output type");
                        }
                    }
                }
        ...
        template<typename T>
        void search(const node_type* n, const Segment2D& seg, COOR_TYPE radius, T& result) const {
            if (n != NULL) {
                if (n->leaf) {
                    if (float d = seg.dist({ n->x, n->y }); d <= radius) {
                        if constexpr (std::is_same_v<T, std::vector<const node_type*>>) {
                            result.push_back(n);
                        }
                        else if constexpr (std::is_same_v<T, std::multimap<float, const node_type*>>) {
                ...
                else if (intersect(n, seg, radius) && n->node_size > 0) {
                    for (int i = 0; i < 4; i++) {
                        if (n->child[i]) {
                            search(n->child[i], seg, radius, result);
                        }
                    }
                }
            }
